package com.yazhi.superswiperefreshlayout;
...
public class SuperSwipeRefreshLayout extends ViewGroup implements NestedScrollingParent,
        NestedScrollingChild, AbsListView.OnScrollListener {

    private Context mContext;
    private ListView mListView;
    boolean isLoading; //是否正在加载中
    private int mYPress; //按下时的y坐标
    private int mYRelease; //抬起时的y坐标
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            if (mRefreshing) {
                ...
                if (isUp) {
                    mCurrentTargetOffsetTop = UpRefreshingDistance;
                } else {
                    mCurrentTargetOffsetTop = mCircleView.getTop();
                }
            } else {
    ...
    public void setOnRefreshListener(OnRefreshListener listener) {
        mListener = listener;
        if (refreshWhenCreate) { //允许在布局进入时就刷新，则回调
...
//    }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float v = mAFloat * mMeasuredHeight;
        UpRefreshingDistance = (int) v;
